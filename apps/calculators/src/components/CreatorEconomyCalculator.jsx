...
import { AlertCircle, CheckCircle2, IndianRupee, MousePointerClick, Send, Target, Users } from 'lucide-react';
...
const CreatorEconomyCalculator = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      <CalculatorLayout>
        <CalculatorHeader
          namespace="CreatorEconomyCalculator"
          title="Creator Economy Quoter"
          icon={Users}

          onReset={() => {
            resetPersistedState('CreatorEconomyCalculator');
          }}
        />
        ...
        <div className="lg:col-span-7 space-y-6">
          <ResultsAnalysis aria-live="polite">
            ...
            <div
              className={`p-4 mb-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-start gap-4 ${results.realityCheck.isRealistic ? 'bg-green-100' : 'bg-yellow-100'}`}
            >
              ...
              <div>
                ...
                {!results.realityCheck.isRealistic && results.realityCheck.requiredAudienceToHitGoal && (
                  <p className="mt-2 text-sm font-bold text-gray-600">
                    To hit this goal at max industry rates, you need ~
                    {results.realityCheck.requiredAudienceToHitGoal.toLocaleString()} total audience.
                  </p>
                )}
              </div>
            </div>
              ...
              <Card title="Recommended Quote Tiers">
                <div className="space-y-4">
                  {results.tiers.map((tier, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center p-3 border-2 border-black hover:bg-gray-50 transition-colors"
                    >
                      <div>
                      ...
                      <div className="text-right">
                        <p className="font-black text-xl">
                          ${tier.flatRate.toLocaleString(undefined, { maximumFractionDigits: 0 })} / post
                        </p>
                        <p className="text-sm text-gray-500 font-bold">
                          ${tier.monthlyRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 })} / mo
                        </p>
                      </div>
                    </div>
                  ))}
