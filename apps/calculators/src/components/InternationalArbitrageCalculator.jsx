    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
        ...
        <Card title="Source Country (Home)" icon={Wallet} headerColor="bg-blue-100">
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <Input label="Source Currency" id="sourceCurrency" name="sourceCurrency" value={inputs.sourceCurrency} onChange={handleChange} placeholder="USD" className="font-black" />
              <Input label="Gross Annual Income" id="grossIncome" name="grossIncome" type="number" value={inputs.grossIncome} onChange={handleChange} className="font-black" />
              <Input label="Income Tax Rate (%)" icon={Percent} id="sourceTaxRate" name="sourceTaxRate" type="number" value={inputs.sourceTaxRate} onChange={handleChange} className="font-black" />
              <Input label="Annual Living Expenses" id="sourceExpenses" name="sourceExpenses" type="number" value={inputs.sourceExpenses} onChange={handleChange} className="font-black" />
            </div>
          </div>
        </Card>
        ...
        <Card title="Target Country (Destination)" icon={PlaneTakeoff} headerColor="bg-green-100">
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4 mb-6">
              <Input label="Target Currency" id="targetCurrency" name="targetCurrency" value={inputs.targetCurrency} onChange={handleChange} placeholder="THB" className="font-black" />
              <Input label="Exchange Rate" id="exchangeRate" name="exchangeRate" type="number" value={inputs.exchangeRate} onChange={handleChange} className="font-black" tooltip={`1 ${inputs.sourceCurrency} = X ${inputs.targetCurrency}`} />
              <Input label="Target Tax Rate (%)" icon={Percent} id="targetTaxRate" name="targetTaxRate" type="number" value={inputs.targetTaxRate} onChange={handleChange} className="font-black" />
              <Input label="Annual Expenses (Local)" id="targetExpenses" name="targetExpenses" type="number" value={inputs.targetExpenses} onChange={handleChange} className="font-black" />
            </div>
            
            <h4 className="text-sm font-bold border-b-2 border-black pb-2 mb-4">Transfer & Platform Fees</h4>
            <div className="grid grid-cols-2 gap-4">
              <Input label="Platform Fee (%)" icon={Percent} id="platformFeeRate" name="platformFeeRate" type="number" value={inputs.platformFeeRate} onChange={handleChange} className="font-black" />
              <Input label="Forex Spread (%)" icon={Percent} id="forexSpreadRate" name="forexSpreadRate" type="number" value={inputs.forexSpreadRate} onChange={handleChange} className="font-black" />
              <Input label="Fixed Transfer Fee" id="transferFeeFixed" name="transferFeeFixed" type="number" value={inputs.transferFeeFixed} onChange={handleChange} className="font-black" />
              <Input label="Transfers Per Year" id="transfersPerYear" name="transfersPerYear" type="number" value={inputs.transfersPerYear} onChange={handleChange} className="font-black" />
            </div>
          </div>
        </Card>
      </div>
        ...
        <ResultsAnalysis title="Arbitrage Results">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 flex flex-col justify-between">
              <MetricDisplay title="Annual Savings (Source)" value={formatCurrency(source.savings, inputs.sourceCurrency)} />
            </div>
            <div className="bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 flex flex-col justify-between">
              <MetricDisplay title="Equivalent Savings (Target)" value={formatCurrency(target.savingsConverted, inputs.sourceCurrency)} />
            </div>
            <div className="bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 flex flex-col justify-between">
              <MetricDisplay title="Time to 1 Yr Source Savings" value={comparison.monthsToReachSourceAnnual > 0 ? `${comparison.monthsToReachSourceAnnual.toFixed(1)} Months` : 'N/A'} />
            </div>
          </div>
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          ...
          The true cost of living in a cheaper country is hidden in platform fees, forex spreads, double taxation, and transfer costs. Always calculate the 'Leakage' before booking a one-way flight.
        </p>
      </Footer>
    </div>
  );
