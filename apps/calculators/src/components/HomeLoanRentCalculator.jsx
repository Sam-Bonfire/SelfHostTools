...
import { AlertCircle, Home, IndianRupee, Key, Landmark, Table as TableIcon, TrendingUp } from 'lucide-react';
...
export default function HomeLoanRentCalculator() {
  const structuredData = {
    ...
    description:
      'Compare the true financial impact of buying vs renting a home including opportunity cost and tax benefits.',
  ...
  const [rentInflation, setRentInflation] = usePersistedState(
    'HomeLoanRentCalculator',
    'rentInflation',
    macroData.inflation.rent
  );
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="HomeLoanRentCalculator"
            icon={Home}
            title="Buy vs Rent Hub"

            onReset={() => {
              resetPersistedState('HomeLoanRentCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  onBlur={() => !propertyValue && setPropertyValue(0)}
                ...
                <Input
                  ...
                  onBlur={() => !downPayment && setDownPayment(0)}
                  className="font-black"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Input
                  label="Tenure (Yrs)"
                  icon={<span className="text-[10px] font-black text-gray-400">Yr</span>}
                  id="loan-tenure"
                  type="number"
                  value={loanTenure}
                  onChange={(e) => setLoanTenure(e.target.value)}
                  onBlur={() => !loanTenure && setLoanTenure(0)}
                  className="font-black"
                />
              </div>
              <Input
                ...
                onBlur={() => !propertyAppreciation && setPropertyAppreciation(0)}
                className="font-black"
              />
            </div>
          </Card>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  onBlur={() => !monthlyRent && setMonthlyRent(0)}
                  className="font-black"
                />
                <Input
                  ...
                  type="number"
                  value={rentInflation}
                  onChange={(e) => setRentInflation(e.target.value)}
                  onBlur={() => !rentInflation && setRentInflation(0)}
                  className="font-black"
                />
              </div>
            </div>
          ...
          <Card
            title="Reality Factors"
            icon={<AlertCircle className="w-5 h-5 md:w-6 md:h-6 text-red-600" />}
              ...
              {investDifference && (
                <div className="pl-8 animate-in zoom-in-95">
                  <Input
                    ...
                    onBlur={() => !equityReturn && setEquityReturn(0)}
                    className="h-8 border-green-600 font-black"
                  />
                </div>
              )}
              <Checkbox
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-6">
          <ResultsAnalysis
            ...
            headerElements={
              <div className="flex gap-2">
                <span
                  className={`text-[10px] font-black px-2 py-1 border-2 border-black ${results.winner === 'Buy' ? 'bg-green-500 text-white' : 'bg-white text-black'}`}
                >
                  BUY
                </span>
                <span
                  className={`text-[10px] font-black px-2 py-1 border-2 border-black ${results.winner === 'Rent' ? 'bg-green-500 text-white' : 'bg-white text-black'}`}
                >
                  RENT
                </span>
              </div>
          ...
          >
            ...
            <div className="bg-black text-white p-6 border-4 border-black shadow-[4px_4px_0px_0px_rgba(255,222,89,1)]">
              <h3 className="text-xs font-black uppercase text-yellow-300 mb-2 tracking-widest">
                      ...
                      <tbody className="divide-y-2 divide-gray-100 text-black">
                        {schedule.map((row) => (
                          <tr key={row.label} className="hover:bg-yellow-50 transition-colors">
                            ...
                            <td
                              className={`p-2 text-right font-black ${row.buyWealth > row.rentWealth ? 'text-blue-600' : 'text-purple-600'}`}
                            >
