...
import { Activity, AlertTriangle, IndianRupee, Info, Settings, TrendingUp } from 'lucide-react';
...
export default function LifestyleCreepCalculator() {
  ...
  const [annualRaisePercent, setAnnualRaisePercent] = usePersistedState(
    'LifestyleCreepCalculator',
    'annualRaisePercent',
    10
  );
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      <CalculatorLayout>
        <CalculatorHeader
          namespace="LifestyleCreepCalculator"
          title="Lifestyle Creep Simulator"
          icon={
            <Activity
              className="w-8 h-8 md:w-10 md:h-10 text-rose-500"
              onReset={() => {
                resetPersistedState('LifestyleCreepCalculator');
              }}
            />
          }
          color="bg-rose-100"
        ...
        <>
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card
              title="Variables"
              icon={Settings}
              className="p-6 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-white"
            >
              <div className="space-y-5">
                ...
                <div className="pt-4 border-t-2 border-dashed border-gray-300">
                  ...
                  <div className="mt-6 p-4 bg-rose-50 border-2 border-black">
                    <label
                      htmlFor="raiseInvestedPercent"
                      className="block text-sm font-black uppercase text-gray-700 mb-2 flex items-center gap-1"
                    >
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="p-4 bg-emerald-50 border-2 border-emerald-200 rounded">
                  <MetricDisplay
                    title="Scenario A (100% Invested)"
                    value={formatCurrency(summary.finalBalanceA)}
                    color="text-emerald-600"
                  />
                </div>
                <div className="p-4 bg-blue-50 border-2 border-blue-200 rounded">
                  <MetricDisplay
                    title={`Your Plan (${raiseInvestedPercent}% Invested)`}
                    value={formatCurrency(summary.finalBalanceB)}
                    color="text-blue-600"
                  />
                </div>
                <div className="p-4 bg-rose-50 border-2 border-rose-200 rounded">
                  <MetricDisplay
                    title="Scenario C (0% Invested)"
                    value={formatCurrency(summary.finalBalanceC)}
                    color="text-rose-600"
                  />
                </div>
              </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={schedule} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    <Area
                      type="monotone"
                      dataKey="balanceB"
                      name={`Your Plan (${raiseInvestedPercent}%)`}
                      stroke="#3b82f6"
                      fillOpacity={0.3}
                      fill="#3b82f6"
                    />
