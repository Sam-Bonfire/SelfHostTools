...
import { AlertCircle, Briefcase, Calendar, Hash, IndianRupee, Lock, TrendingUp, Unlock } from 'lucide-react';
...
export default function GoldenHandcuffsCalculator() {
  ...
  const calculate = useCallback(() => {
    const currentStats = calculateAnnualizedComp({
      ...
      equity: currentEquity.map((e) => ({
        ...e,
        count: Number(e.count),
        price: Number(e.price),
        strike: Number(e.strike || 0),
        vestingYears: Number(e.vestingYears)
      }))
    });
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="GoldenHandcuffsCalculator"
            icon={Lock}
            title="Golden Handcuffs"

            onReset={() => {
              resetPersistedState('GoldenHandcuffsCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-6 space-y-8">
          {/* 1. CURRENT JOB */}
          <Card
            title="Current Role (The Handcuffs)"
            icon={Briefcase}
          ...
          >
            <div className="p-4 space-y-4">
              ...
              <div className="border-t-2 border-dashed border-gray-300 pt-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    ...
                    className="text-[10px] py-1 px-2 h-auto border-2"
                  >
                    Add Grant
                  </Button>
                </div>
                  ...
                  {currentEquity.map((grant, idx) => (
                    <div key={grant.id} className="bg-blue-50 p-2 border-2 border-black relative">
                      <button
                        ...
                        className="absolute -top-2 -right-2 bg-red-500 text-white w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold border-2 border-black hover:bg-red-600"
                      >
                        ×
                      </button>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-2">
                        <select
                          id={`current-equity-type-${grant.id}`}
                          aria-label="Equity Type"
                          value={grant.type}
                          onChange={(e) => updateEquity(setCurrentEquity, grant.id, 'type', e.target.value)}
                          className="border-2 border-black text-xs font-bold p-1 bg-white text-black"
                        >
                          <option value="RSU">RSU</option>
                          <option value="Option">Option</option>
                        </select>
                        <Input
                          id={`current-equity-count-${grant.id}`}
                          aria-label="Units Count"
                          placeholder="Count"
                          icon={Hash}
                          type="number"
                          value={grant.count}
                          onChange={(e) => updateEquity(setCurrentEquity, grant.id, 'count', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                        <Input
                          id={`current-equity-vesting-${grant.id}`}
                          aria-label="Vesting Years"
                          placeholder="Vesting Yrs"
                          icon={Calendar}
                          type="number"
                          value={grant.vestingYears}
                          onChange={(e) => updateEquity(setCurrentEquity, grant.id, 'vestingYears', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <Input
                          id={`current-equity-price-${grant.id}`}
                          aria-label="Grant Price"
                          placeholder={grant.type === 'RSU' ? 'Price' : 'Current Price'}
                          icon={IndianRupee}
                          type="number"
                          value={grant.price}
                          onChange={(e) => updateEquity(setCurrentEquity, grant.id, 'price', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                        {grant.type === 'Option' && (
                          <Input
                            id={`current-equity-strike-${grant.id}`}
                            aria-label="Strike Price"
                            placeholder="Strike Price"
                            icon={IndianRupee}
                            type="number"
                            value={grant.strike}
                            onChange={(e) => updateEquity(setCurrentEquity, grant.id, 'strike', e.target.value)}
                            className="h-8 text-xs font-black"
                          />
                        )}
                      </div>
              ...
              <div className="border-t-2 border-dashed border-gray-300 pt-4">
                <h3 className="text-[10px] font-black uppercase text-red-600 mb-2 flex items-center gap-1">
                  <AlertCircle className="w-3 h-3" /> The Clause (Clawbacks)
                </h3>
                <div>
                  <Input
                    id="clawback-amount"
                    ...
                    className="border-red-600 font-black text-red-600"
                  />
                ...
                <div className="space-y-3">
                  {newEquity.map((grant, idx) => (
                    <div key={grant.id} className="bg-green-50 p-2 border-2 border-black relative">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-2">
                        <select
                          id={`new-equity-type-${grant.id}`}
                          aria-label="Equity Type"
                          value={grant.type}
                          onChange={(e) => updateEquity(setNewEquity, grant.id, 'type', e.target.value)}
                          className="border-2 border-black text-xs font-bold p-1 bg-white text-black"
                        >
                          <option value="RSU">RSU</option>
                          <option value="Option">Option</option>
                        ...
                        <Input
                          id={`new-equity-count-${grant.id}`}
                          aria-label="Units Count"
                          placeholder="Count"
                          icon={Hash}
                          type="number"
                          value={grant.count}
                          onChange={(e) => updateEquity(setNewEquity, grant.id, 'count', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                        <Input
                          id={`new-equity-vesting-${grant.id}`}
                          aria-label="Vesting Years"
                          placeholder="Vesting Yrs"
                          icon={Calendar}
                          type="number"
                          value={grant.vestingYears}
                          onChange={(e) => updateEquity(setNewEquity, grant.id, 'vestingYears', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <Input
                          id={`new-equity-price-${grant.id}`}
                          aria-label="Grant Price"
                          placeholder={grant.type === 'RSU' ? 'Price' : 'Est. Value'}
                          icon={IndianRupee}
                          type="number"
                          value={grant.price}
                          onChange={(e) => updateEquity(setNewEquity, grant.id, 'price', e.target.value)}
                          className="h-8 text-xs font-black"
                        />
                        {grant.type === 'Option' && (
                          <Input
                            id={`new-equity-strike-${grant.id}`}
                            aria-label="Strike Price"
                            placeholder="Strike Price"
                            icon={IndianRupee}
                            type="number"
                            value={grant.strike}
                            onChange={(e) => updateEquity(setNewEquity, grant.id, 'strike', e.target.value)}
                            className="h-8 text-xs font-black"
                          />
                        )}
                      </div>
        ...
        <div className="lg:col-span-12 xl:col-span-6 space-y-6">
          <ResultsAnalysis
            ...
            headerElements={
              results.breakEvenYear ? (
                <span className="bg-black text-white text-xs font-black px-2 py-1 border-2 border-black uppercase">
                  Break Even: Year {results.breakEvenYear}
                </span>
              ) : (
                <span className="bg-red-500 text-white text-xs font-black px-2 py-1 border-2 border-black uppercase">
                  Never Catch Up
                </span>
              )
            }
          >
              ...
              <div className="bg-yellow-100 border-2 border-black p-3 flex items-start gap-3">
                ...
                <div className="space-y-1">
                  {results.alerts.map((alert, i) => (
                    <p key={i} className="text-xs font-bold text-red-800">
                      {alert.message}
                    </p>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div
                className={`p-3 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${results.isNewJobBetter ? 'bg-green-100' : 'bg-red-50'}`}
              >
                <p
                  className={`text-[9px] font-black uppercase mb-1 ${results.isNewJobBetter ? 'text-green-800' : 'text-red-800'}`}
                >
                  New True Annual Comp
                </p>
                <p className={`text-xl font-black ${results.isNewJobBetter ? 'text-green-800' : 'text-red-800'}`}>
                  {formatCurrency(results.newTC)}
                </p>
              </div>
            </div>
