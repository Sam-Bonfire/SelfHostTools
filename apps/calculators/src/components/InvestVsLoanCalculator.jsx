...
import { IndianRupee, Info, Landmark, Percent, PiggyBank, Plus, Scale, Table as TableIcon, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  const [investmentTaxRate, setInvestmentTaxRate] = usePersistedState(
    'InvestVsLoanCalculator',
    'investmentTaxRate',
    10
  );
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <CalculatorHeader
          namespace="InvestVsLoanCalculator"
          icon={Scale}
          title="Invest or Pay Off Debt?"

          onReset={() => {
            resetPersistedState('InvestVsLoanCalculator');
          }}
        />
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  This amount fuels either your Investments or Debt Repayment.
                </p>
              </div>
                ...
                <div key={loan.id} className="relative p-4 bg-gray-50 border-2 border-black rounded-sm">
                  ...
                  {loan.isTaxDeductible && (
                    <div className="mt-2 text-xs bg-blue-100 p-1 border border-blue-300">
                      Effective Rate: <strong>{(loan.rate * (1 - userTaxBracket / 100)).toFixed(2)}%</strong> (assuming{' '}
                      {userTaxBracket}% bracket)
                    </div>
                  )}
                </div>
              ))}
          ...
          <ResultsAnalysis>
            {results && (
              <>
                {/* Verdict Card */}
                <div
                  className={`border-4 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] mb-6 text-white text-center transform rotate-1
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                    <h4 className="font-black text-lg text-green-700 bg-green-100 px-2 py-1 inline-block border-2 border-black mb-4">
                      Invest Strategy
                    </h4>
                    <div className="space-y-4">
                      <MetricDisplay
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <p className="font-bold">
                            {Math.floor(results.results.investStrategy.debtFreeMonth / 12)}y{' '}
                            {results.results.investStrategy.debtFreeMonth % 12}m
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-gray-500">Interest Paid</p>
                          <p className="font-bold text-red-600">
                            {formatCurrency(results.results.investStrategy.totalInterestPaid)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                    <h4 className="font-black text-lg text-red-700 bg-red-100 px-2 py-1 inline-block border-2 border-black mb-4">
                      Payoff Strategy
                    </h4>
                    <div className="space-y-4">
                      <MetricDisplay
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          <p className="font-bold text-gray-500">Debt Free In</p>
                          <p className="font-bold">
                            {Math.floor(results.results.payoffStrategy.debtFreeMonth / 12)}y{' '}
                            {results.results.payoffStrategy.debtFreeMonth % 12}m
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-gray-500">Interest Paid</p>
                          <p className="font-bold text-red-600">
                            {formatCurrency(results.results.payoffStrategy.totalInterestPaid)}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-8 bg-yellow-200 border-4 border-black p-4 flex gap-4 items-start">
                  ...
                  <div className="text-xs md:text-sm font-medium">
                    ...
                    <ul className="list-disc ml-4 space-y-1 mt-1">
                      ...
                      <li>
                        <strong>Risk Note:</strong> Paying off debt is a guaranteed{' '}
                        {Math.max(
                          ...loans.map((l) => l.rate * (l.isTaxDeductible ? 1 - userTaxBracket / 100 : 1)),
                          0
                        ).toFixed(2)}
                        % return. Investing {investmentReturn}% is volatile.
                      </li>
                    </ul>
                  </div>
                </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                          ...
                          <tbody className="divide-y-2 divide-gray-100 bg-white">
                            {results.results.investStrategy.history
                              .filter((_, i) => i % 12 === 11)
                              .map((row, i) => {
                                const payoffRow = results.results.payoffStrategy.history[i * 12 + 11];
                                const payoffNW = payoffRow ? payoffRow.netWorth : 0;
                                ...
                                return (
                                  <tr key={i} className="hover:bg-yellow-50">
                                    <td className="p-3 font-black text-black border-r border-gray-200">
                                      Year {row.year}
                                    </td>
                                    <td className="p-3 font-bold text-black border-r border-gray-200">
                                      {formatCurrency(row.netWorth)}
                                    </td>
                                    <td className="p-3 font-bold text-black border-r border-gray-200">
                                      {formatCurrency(payoffNW)}
                                    </td>
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          <strong>Disclaimer:</strong> This calculation uses expected market returns, but market returns are volatile
          while debt interest is guaranteed.
          <br className="md:hidden" />
          Always consider your psychological comfort—sometimes peace of mind from zero debt outweighs mathematical
          optimization.
        </p>
      </Footer>
    </div>
