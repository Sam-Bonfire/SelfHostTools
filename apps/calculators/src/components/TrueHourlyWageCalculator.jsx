...
export default function TrueHourlyWageCalculator() {
  const structuredData = {
    ...
    description:
      'Calculate your real hourly wage after accounting for unpaid overtime, commute, and work-related expenses.',
    brand: { '@type': 'Brand', name: 'Calculators Hub' }
  };

  // --- INPUTS ---
  const [annualGrossSalary, setAnnualGross] = usePersistedState(
    'TrueHourlyWageCalculator',
    'annualGrossSalary',
    1200000
  ); // 12 LPA default
  ...
  const [vacationWeeksPerYear, setVacationWeeks] = usePersistedState(
    'TrueHourlyWageCalculator',
    'vacationWeeksPerYear',
    2
  );
  const [standardHoursPerWeek, setStandardHours] = usePersistedState(
    'TrueHourlyWageCalculator',
  ...
  // Time Leaks
  const [commuteOneWayMinutes, setCommuteMinutes] = usePersistedState(
    'TrueHourlyWageCalculator',
    'commuteOneWayMinutes',
    45
  );
  const [groomingMinutesDaily, setGroomingMinutes] = usePersistedState(
    'TrueHourlyWageCalculator',
    'groomingMinutesDaily',
    20
  );
  const [decompressionMinutesDaily, setDecompressionMinutes] = usePersistedState(
    'TrueHourlyWageCalculator',
    'decompressionMinutesDaily',
    30
  );
  const [unpaidOvertimeHoursPerWeek, setUnpaidOvertime] = usePersistedState(
    'TrueHourlyWageCalculator',
  ...
  const [monthlyConvenienceRen, setConvenienceCost] = usePersistedState(
    'TrueHourlyWageCalculator',
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="TrueHourlyWageCalculator"
            icon={Briefcase}
            title="True Hourly Wage"

            onReset={() => {
              resetPersistedState('TrueHourlyWageCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <Card title="Time Leaks" icon={Clock} headerColor="bg-orange-100">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Input
                    ...
                    id="unpaid-overtime"
                    type="number"
                    value={unpaidOvertimeHoursPerWeek}
                    onChange={(e) => setUnpaidOvertime(e.target.value)}
                    icon={Clock}
                    placeholder="Hrs"
                    className="font-black w-full"
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    ...
                    id="grooming-minutes"
                    type="number"
                    value={groomingMinutesDaily}
                    onChange={(e) => setGroomingMinutes(e.target.value)}
                    icon={Scissors}
                ...
                <div>
                  <Input
                    label="Decompression"
                    id="decompression-minutes"
                    type="number"
                    value={decompressionMinutesDaily}
                    onChange={(e) => setDecompressionMinutes(e.target.value)}
                    icon={Wind}
                    placeholder="Mins/day"
                    className="font-black w-full"
                  />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 border-t-2 border-black/10 pt-4">
                <div>
                  <Input
                    label="Week Days"
                    id="working-days"
                    ...
                    icon={Calendar}
                    className="h-8 font-black text-sm w-full"
                  />
                </div>
                <div>
                  <Input
          ...
          <Card title="Money Leaks (Post-Tax)" icon={AlertTriangle} headerColor="bg-red-100">
            <div className="space-y-4">
              <div>
                <Input
                  label="Daily Commute Cost"
                  id="commute-cost"
                  type="number"
                  value={commuteDailyCost}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    ...
                    id="convenience-cost"
                    type="number"
                    value={monthlyConvenienceRen}
                    onChange={(e) => setConvenienceCost(e.target.value)}
                    icon={Coffee}
                    className="font-black w-full"
                  />
                  <span className="text-[9px] text-gray-500 font-bold uppercase block mt-1">
                    Per Month (Takeout, Maid)
                  </span>
                </div>
                <div>
                  <Input
                    ...
                    id="health-cost"
                    type="number"
                    value={monthlyHealthren}
                    onChange={(e) => setHealthCost(e.target.value)}
                    icon={Heart}
                    className="font-black w-full"
                  />
                  <span className="text-[9px] text-gray-500 font-bold uppercase block mt-1">
                    Per Month (Therapy, Physio)
                  </span>
                </div>
              </div>
            </div>
          ...
          {results && (
            <ResultsAnalysis>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* NOMINAL RATE */}
                <Card
                  className="p-4 border-4 border-black bg-gray-50 flex flex-col justify-between h-full"
                  animate={false}
                >
                  <div>
                ...
                >
                  <Card
                    className="p-4 border-4 border-black bg-yellow-300 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-full"
                    animate={false}
                  >
                    <div>
              ...
              <Card title="The Audit" icon={FileText} headerColor="bg-black !text-white">
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 mb-8">
                    <h4 className="text-xs font-black uppercase border-b-2 border-black pb-2 mb-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-1 h-32 items-end mt-4 pl-4 border-l-2 border-black border-dashed">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-end group">
                          ...
                          <div
                            className="w-full bg-red-400 border border-black transition-all group-hover:bg-red-500"
                            style={{
                              height: `${(results.financials.annualTax / results.financials.annualGross) * 100}%`,
                              minHeight: '4px'
                            }}
                          ></div>
                          <span className="text-[9px] font-bold mt-1">
                            -{formatCurrency(results.financials.annualTax)}
                          </span>
                        </div>
                        {/* Commute Money */}
                        <div className="flex-1 flex flex-col items-center justify-end group">
                          ...
                          <div
                            className="w-full bg-orange-400 border border-black transition-all group-hover:bg-orange-500"
                            style={{
                              height: `${(results.leaks.commuteCost / results.financials.annualGross) * 100}%`,
                              minHeight: '4px'
                            }}
                          ></div>
                          <span className="text-[9px] font-bold mt-1">
                            -{formatCurrency(results.leaks.commuteCost)}
                          </span>
                        </div>
                        {/* Life Cost */}
                        <div className="flex-1 flex flex-col items-center justify-end group">
                          ...
                          <div
                            className="w-full bg-yellow-400 border border-black transition-all group-hover:bg-yellow-500"
                            style={{
                              height: `${((results.leaks.convenienceCost + results.leaks.healthCost) / results.financials.annualGross) * 100}%`,
                              minHeight: '4px'
                            }}
                          ></div>
                          <span className="text-[9px] font-bold mt-1">
                            -{formatCurrency(results.leaks.convenienceCost + results.leaks.healthCost)}
                          </span>
                        </div>
                      </div>
      ...
      <div className="mt-8 p-6 border-4 border-black bg-yellow-50 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <p className="text-sm font-bold text-gray-800">
          <strong className="text-black">Reality Check:</strong> If your True Hourly Wage is lower than your local
          fast-food rate, it might be time to discuss remote work or a raise.
        </p>
      </div>
