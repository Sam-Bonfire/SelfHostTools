    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
          <Card title="Financial Profile" icon={Briefcase} headerColor="bg-blue-100" className="p-0 border-4 border-black">
            <div className="p-4 space-y-4">
              <div>
                <Tooltip content="Expenses needed for family survival (Groceries, bills, fees, etc.)" className="w-full">
                  <Input label="Monthly Household Expenses" icon={IndianRupee} id="monthlyExpense" type="number" value={monthlyExpense} onChange={e => setMonthlyExpense(e.target.value)} onBlur={() => !monthlyExpense && setMonthlyExpense(0)} className="font-black w-full" />
                </Tooltip>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Tooltip content="Years your family needs support (e.g. until youngest child starts earning)">
                    <Input label="Years to Replace" icon={Calendar} id="yearsToReplace" type="number" value={yearsToReplace} onChange={e => setYearsToReplace(e.target.value)} onBlur={() => !yearsToReplace && setYearsToReplace(0)} className="font-black" />
                  </Tooltip>
                </div>
                <div>
                  <Tooltip content="Expected annual price rise (Standard is 6%)">
                    <Input label="Avg. Inflation (%)" icon={Percent} id="inflationRate" type="number" value={inflationRate} onChange={e => setInflationRate(e.target.value)} onBlur={() => !inflationRate && setInflationRate(0)} className="font-black" />
                  </Tooltip>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 border-t-2 border-black/10 pt-4 mt-2">
                <div>
                  <Tooltip content="Safe return on the insurance money (e.g. FD/Debt Fund)">
                    <Input label="Expected Return (%)" icon={Percent} id="investmentReturn" type="number" value={investmentReturn} onChange={e => setInvestmentReturn(e.target.value)} onBlur={() => !investmentReturn && setInvestmentReturn(0)} className="font-black" />
                  </Tooltip>
                </div>
                <div>
                  <Tooltip content="% of expense that is purely for YOU (stopped after death)">
                    <Input label="Self-Spend (%)" icon={Percent} id="personalShare" type="number" value={personalShare} onChange={e => setPersonalShare(e.target.value)} onBlur={() => !personalShare && setPersonalShare(0)} className="font-black" />
                  </Tooltip>
                </div>
              </div>
            </div>
          ...
          <Card title="Future Big Goals" icon={GraduationCap} headerColor="bg-orange-50" className="p-0 border-4 border-black">
            <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Tooltip content="Current cost of this goal">
                        <Input icon={IndianRupee} id={`goal-amount-${g.id}`} aria-label={`Cost for ${g.name}`} type="number" value={g.amount} onChange={e => updateGoal(g.id, 'amount', e.target.value)} onBlur={() => !g.amount && updateGoal(g.id, 'amount', 0)} className="h-8 text-xs border-black font-black" />
                      </Tooltip>
                    </div>
                    <div>
                      <Tooltip content="Years remaining until this goal is due">
                        <Input icon={Calendar} id={`goal-years-${g.id}`} aria-label={`Years away for ${g.name}`} type="number" value={g.yearsAway} onChange={e => updateGoal(g.id, 'yearsAway', e.target.value)} onBlur={() => !g.yearsAway && updateGoal(g.id, 'yearsAway', 0)} className="h-8 text-xs border-black font-black" placeholder="Yrs" />
                      </Tooltip>
                    </div>
                  </div>
                </div>
          ...
          <Card title="Liabilities & Assets" icon={Landmark} headerColor="bg-red-50" className="p-0 border-4 border-black">
            <div className="p-4 space-y-4">
              <div>
                <Tooltip content="Outstanding principal on all loans" className="w-full">
                  <Input label="Total Loans (Home/Car/Edu)" icon={IndianRupee} id="liabilities" type="number" value={liabilities} onChange={e => setLiabilities(e.target.value)} onBlur={() => !liabilities && setLiabilities(0)} className="font-black w-full" />
                </Tooltip>
              </div>
              <div>
                <Tooltip content="Money available immediately. Do NOT include property." className="w-full">
                  <Input label="Current Liquid Assets (Fds/Gold/MF)" icon={IndianRupee} id="existingAssets" type="number" value={existingAssets} onChange={e => setAssets(e.target.value)} onBlur={() => !existingAssets && setAssets(0)} className="font-black w-full" />
                </Tooltip>
              </div>
              <div className="pt-2 border-t-2 border-black/10">
                <Tooltip content="Sum Assured of all current Term/LIC policies" className="w-full">
                  <Input label="Existing Life Insurance Policy" icon={ShieldCheck} id="currentInsurance" type="number" value={currentInsurance} onChange={e => setCurrentInsurance(e.target.value)} onBlur={() => !currentInsurance && setCurrentInsurance(0)} className="border-blue-600 font-black text-blue-700 w-full" />
                </Tooltip>
              </div>
            </div>
          </Card>
