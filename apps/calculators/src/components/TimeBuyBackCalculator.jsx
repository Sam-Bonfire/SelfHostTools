...
export default function TimeBuyBackCalculator() {
  const structuredData = {
    ...
    description:
      'Evaluate if delegation is financially and psychologically optimal. Reclaim your time using true net wage and surcharge multipliers.',
    brand: {
      '@type': 'Brand',
  ...
  const [salariedHoursPerWeek, setSalariedHoursPerWeek] = usePersistedState(
    'TimeBuyBackCalculator',
    'salariedHoursPerWeek',
    40
  );
  const [commuteHoursPerWeek, setCommuteHoursPerWeek] = usePersistedState(
    'TimeBuyBackCalculator',
    'commuteHoursPerWeek',
    5
  );
  const [unpaidOvertimeHours, setUnpaidOvertimeHours] = usePersistedState(
    'TimeBuyBackCalculator',
    'unpaidOvertimeHours',
    5
  );
  ...
  const [reinvestmentType, setReinvestmentType] = usePersistedState(
    'TimeBuyBackCalculator',
    'reinvestmentType',
    'leisure'
  ); // 'leisure', 'upskilling', 'work'
  ...
  const handleExportPDF = () => {
    downloadPDF({
      ...
      schedule: [
        ...
        {
          label: 'Time Value Reinvested',
          principal: 0,
          interest: results.delegation.reinvestmentValue,
          balance: results.delegation.reinvestmentValue
        },
        {
          label: 'Net Delegation Gain',
          principal: 0,
          interest: results.delegation.delegationGain,
          balance: results.delegation.delegationGain
        }
      ]
    });
  };
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="TimeBuyBackCalculator"
            icon={Zap}
            title="Time Buy-Back & Task Delegate Realist"

            onReset={() => {
              resetPersistedState('TimeBuyBackCalculator');
            }}
          />
        </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Input
                        ...
                        onBlur={() => !hourlyRate && setHourlyRate(0)}
                    ...
                    <div>
                      <Input
                        id="hoursPerWeek"
                        label="Weekly Billed Hours"
                        icon={Clock}
                        type="number"
                        value={hoursPerWeek}
                        onChange={(e) => setHoursPerWeek(e.target.value)}
                        onBlur={() => !hoursPerWeek && setHoursPerWeek(0)}
                        className="font-black"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Input
                        id="vacationWeeks"
                        label="Vacation (Weeks/Yr)"
                        type="number"
                        value={vacationWeeks}
                        onChange={(e) => setVacationWeeks(e.target.value)}
                        onBlur={() => !vacationWeeks && setVacationWeeks(0)}
                        className="font-black"
                      />
                    </div>
                    <div>
                      <Input
                        id="adminTimePercent"
                        label="Unbilled Admin Time (%)"
                        type="number"
                        value={adminTimePercent}
                        onChange={(e) => setAdminTimePercent(e.target.value)}
                        onBlur={() => !adminTimePercent && setAdminTimePercent(0)}
                        className="font-black"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t-2 border-black/10">
                    <div>
                      <Input
                        ...
                        onBlur={() => !freelanceTaxRate && setFreelanceTaxRate(0)}
                        className="font-black"
                      />
                    </div>
                ...
                <>
                  <div>
                    <Input
                      ...
                      type="number"
                      value={salariedGross}
                      onChange={(e) => setSalariedGross(e.target.value)}
                      onBlur={() => !salariedGross && setSalariedGross(0)}
                      className="font-black"
                    />
                    <input
                      type="range"
                      ...
                      max={6000000}
                      step={50000}
                      value={salariedGross || 0}
                      onChange={(e) => setSalariedGross(Number(e.target.value))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Input
                        id="salariedHoursPerWeek"
                        label="Official Hours/Week"
                        type="number"
                        value={salariedHoursPerWeek}
                        onChange={(e) => setSalariedHoursPerWeek(e.target.value)}
                        onBlur={() => !salariedHoursPerWeek && setSalariedHoursPerWeek(0)}
                        className="font-black"
                      />
                    </div>
                    <div>
                      <Input
                        ...
                        type="number"
                        value={salariedTaxRate}
                        onChange={(e) => setSalariedTaxRate(e.target.value)}
                        onBlur={() => !salariedTaxRate && setSalariedTaxRate(0)}
                        className="font-black"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t-2 border-black/10">
                    <div>
                      <Input
                        ...
                        label="Weekly Commute (Hrs)"
                        type="number"
                        value={commuteHoursPerWeek}
                        onChange={(e) => setCommuteHoursPerWeek(e.target.value)}
                        onBlur={() => !commuteHoursPerWeek && setCommuteHoursPerWeek(0)}
                        className="font-black"
                      />
                    </div>
                    <div>
                      <Input
                        ...
                        label="Weekly Overtime (Hrs)"
                        type="number"
                        value={unpaidOvertimeHours}
                        onChange={(e) => setUnpaidOvertimeHours(e.target.value)}
                        onBlur={() => !unpaidOvertimeHours && setUnpaidOvertimeHours(0)}
                        className="font-black"
                      />
                    </div>
                  </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    id="taskCost"
                    label="Task Outsource Cost (₹)"
                    icon={IndianRupee}
                    type="number"
                    value={taskCost}
                    onChange={(e) => setTaskCost(e.target.value)}
                    onBlur={() => !taskCost && setTaskCost(0)}
                    className="font-black"
                  />
                </div>
                <div>
                  <Input
                    ...
                    onBlur={() => !hoursSaved && setHoursSaved(0)}
                    className="font-black"
                  />
                </div>
              </div>
              ...
              <div>
                <Input
                  id="energyMultiplier"
                  label="Psychological Energy Multiplier (1x to 3x)"
                  icon={Percent}
                  type="number"
                  step="0.1"
                  value={energyMultiplier}
                  onChange={(e) => setEnergyMultiplier(e.target.value)}
                  onBlur={() => !energyMultiplier && setEnergyMultiplier(1.0)}
                  className="font-black"
                />
                <p className="text-[10px] text-gray-500 mt-2 font-bold uppercase leading-none tracking-tight">
                  Fatigue score: apply high multiplier for tasks you highly dislike
                </p>
                <input
                  type="range"
                  min={1.0}
                  max={3.0}
                  step={0.1}
                  value={energyMultiplier || 1.0}
                  onChange={(e) => setEnergyMultiplier(Number(e.target.value))}
                  className="w-full mt-3 h-2 bg-gray-200 appearance-none cursor-pointer accent-black"
                  aria-label="Energy Surcharge Slider"
                />
              </div>
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-6">
          <ResultsAnalysis>
            ...
            <div
              className={`border-4 border-black text-white p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row items-center justify-between gap-4 transition-colors duration-300 ${results.delegation.isWorthIt ? 'bg-green-600' : 'bg-red-600'}`}
            >
              ...
              <div className="bg-[#FF6B6B]/10 border-4 border-black p-4 flex items-start gap-4 animate-bounce">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-800 font-bold uppercase mt-1">
                    Your True Net Hourly Wage is zero or negative! Under these baseline parameters, any outsource cost
                    is an outright cash loss. Please audit your income, billable hours, or overhead tax parameters
                    above.
                  </p>
                </div>
              </div>
            ...
            {taskCost > results.baseline.trueNetHourlyWage * hoursSaved && results.delegation.isWorthIt && (
              <div className="bg-[#FF9900]/10 border-4 border-black p-4 flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-sm font-black uppercase text-orange-950 leading-tight">
                    PSYCHOLOGICAL COGNITIVE TRADE-OFF
                  </h4>
                  <p className="text-xs text-orange-800 font-bold uppercase mt-1">
                    This task has a higher outsource cost than your net financial hourly returns, but is marked as WORTH
                    IT purely because of your high Fatigue Multiplier ({energyMultiplier}x). You are technically trading
                    cash for mental relief. Ensure you have the cash reserves to absorb this convenience tax.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="border-4 border-black p-6 bg-yellow-50 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[10px] font-black uppercase text-gray-500">Outsource Task Cost</span>
                    <span className="text-xs font-black">{formatCurrency(taskCost)}</span>
                  </div>
                  <div className="w-full bg-gray-200 border-2 border-black h-6 relative shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (parseFloat(taskCost) / Math.max(1, parseFloat(results.delegation.reinvestmentValue) + parseFloat(taskCost))) * 100)}%`
                      }}
                    ></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[10px] font-black uppercase text-green-600">
                      Reclaimed Time Value ({reinvestmentType.toUpperCase()})
                    </span>
                    <span className="text-xs font-black text-green-600">
                      {formatCurrency(results.delegation.reinvestmentValue)}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 border-2 border-black h-6 relative shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <div
                      className="bg-green-400 h-full border-r-2 border-black"
                      style={{
                        width: `${Math.min(100, (parseFloat(results.delegation.reinvestmentValue) / Math.max(1, parseFloat(results.delegation.reinvestmentValue) + parseFloat(taskCost))) * 100)}%`
                      }}
                    ></div>
                  </div>
                </div>
              ...
              <div className="bg-white border-2 border-black p-3 text-xs font-bold uppercase tracking-tight flex justify-between items-center shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                ...
                <span className="font-black text-green-600">
                  {formatCurrency(results.delegation.reinvestmentValue)} (
                  {formatCurrency(results.delegation.breakEvenCostPerHour)}/hr)
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <DownloadButtons onDownloadPDF={handleExportPDF} onDownloadExcel={handleExportExcel} />
            </div>
          </ResultsAnalysis>
        </div>
      </CalculatorLayout>
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          <strong>Disclaimer:</strong> The Psychological Fatigue Multiplier is a subjective cognitive weight designed to
          quantify unliked labor; actual wealth outcomes are based strictly on take-home billable cash flows. Plan your
          cash purchases responsibly.
        </p>
      </Footer>
    </div>
  );
