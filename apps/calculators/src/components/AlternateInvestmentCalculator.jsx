...
import { Briefcase, Clock, IndianRupee, Pickaxe, ShieldCheck, TrendingDown, TrendingUp } from 'lucide-react';
...
export default function AlternateInvestmentCalculator() {
  const structuredData = {
    ...
    description:
      'Calculate the true ROI of active investments like businesses or rentals by deducting the cost of your time and effort.',
    brand: { '@type': 'Brand', name: 'Calculators Hub' }
  };

  // --- INPUTS ---
  const [initialInvestment, setInitial] = usePersistedState(
    'AlternateInvestmentCalculator',
    'initialInvestment',
    1000000
  );
  const [monthlyContribution, setMonthly] = usePersistedState(
    'AlternateInvestmentCalculator',
    'monthlyContribution',
    0
  );
  const [years, setYears] = usePersistedState('AlternateInvestmentCalculator', 'years', 10);
  const [estReturnRate, setReturnRate] = usePersistedState(
    'AlternateInvestmentCalculator',
    'estReturnRate',
    macroData.returns.equityBenchmark
  );

  // --- REALITY CHECKS ---
  const [isActiveInvestment, setIsActive] = usePersistedState(
    'AlternateInvestmentCalculator',
    'isActiveInvestment',
    false
  );
  ...
  const [benchmarkReturn, setBenchmarkReturn] = usePersistedState(
    'AlternateInvestmentCalculator',
    'benchmarkReturn',
    macroData.returns.equityBenchmark
  );
  const [marketScenario, setMarketScenario] = usePersistedState(
    'AlternateInvestmentCalculator',
    'marketScenario',
    'normal'
  ); // normal, bull, bear
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="AlternateInvestmentCalculator"
            icon={Pickaxe}
            title="The 'Sweat Equity' Truth"

            onReset={() => {
              resetPersistedState('AlternateInvestmentCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
          ...
          >
            ...
            <div className="p-4 border-t-4 border-black/10">
              <Tooltip content="Toggle advanced settings for Tax and Inflation adjustments" className="w-full">
                <Button onClick={() => setShowAdvanced(!showAdvanced)} variant="secondary" className="w-full">
                    ...
                    <div className="grid grid-cols-2 gap-4 p-4 border-2 border-yellow-200 bg-yellow-50 rounded dash-border">
                      <div>
                        <Input
                          id="tax-rate"
                          label="Tax on Gains (%)"
                          tooltip="Capital Gains Tax deducted from the final profit."
                          type="number"
                          value={taxRate}
                          onChange={(e) => setTaxRate(e.target.value)}
                          className="font-black w-full bg-white border-yellow-200 focus:border-yellow-500"
                        />
                      </div>
                      <div>
                        <Input
                          id="inflation-rate"
                          label="Inflation (%)"
                          tooltip="Estimated annual loss of purchasing power (Standard ~6%)."
                          type="number"
                          value={inflationRate}
                      ...
                      <div className="col-span-2 border-t border-yellow-200 pt-3 mt-1">
                        <Input
                          id="benchmark-return"
                          label="Passive Benchmark Return (%)"
                          tooltip="Expected annual return of your 'Lazy' alternative (e.g., NIFTY 50 Index Fund ~12%)."
                          type="number"
                          value={benchmarkReturn}
                          onChange={(e) => setBenchmarkReturn(e.target.value)}
                          className="font-black w-full bg-white border-blue-200 focus:border-blue-500"
                        />
                      </div>
          ...
          <Card
            className="p-0 border-4 border-black mt-6"
            title='The "Active" Tax'
            icon={Clock}
            headerColor="bg-red-50"
          >
            <div className="p-4 space-y-4">
              <div
                className={`p-3 border-2 rounded transition-colors ${isActiveInvestment ? 'bg-red-100 border-red-600' : 'bg-gray-50 border-gray-200'}`}
              >
                <Checkbox
                  label="This is an Active Investment"
                  checked={isActiveInvestment}
                    ...
                    <div className="grid grid-cols-2 gap-4 p-4 border-2 border-red-200 bg-red-50 rounded dash-border">
                      <div>
                        <Input
                          id="active-hours"
                          label="Hours/Week"
                          tooltip="Time you spend managing this investment (e.g., Calling tenants, bookkeeping, sales)."
                          type="number"
                          value={activeHoursPerWeek}
                          onChange={(e) => setHours(e.target.value)}
                          className="font-black w-full bg-white border-red-200 focus:border-red-900"
                        />
                      </div>
                      <div>
                        <Input
                          ...
                          tooltip="What is your time worth? Use your current salary hourly rate or freelance rate."
                          type="number"
        ...
        {results && (
          <div className="lg:col-span-12 xl:col-span-7 space-y-6">
            ...
            >
              {/* BIG REVEAL CARD */}
              <div
                className={`p-6 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center text-center ${results.roi.truePassive < 0 ? 'bg-red-100' : 'bg-green-300'}`}
              >
                <MetricDisplay
                ...
                />
                <div
                  className={`text-sm font-bold uppercase py-1 px-3 rounded border-2 border-black inline-block ${results.roi.truePassive < 0 ? 'bg-red-200 text-red-900' : 'bg-white text-green-900'}`}
                >
              ...
              <div className="grid grid-cols-3 gap-2 md:gap-4">
                ...
                <div
                  className={`p-3 border-4 border-black rounded relative ${results.roi.truePassive < 0 ? 'bg-red-50' : 'bg-green-50'}`}
                >
                  ...
                  <p className="text-lg font-black">
                    {formatCurrency(results.netRealProfitAfterTime + results.totalInvested)}
                  </p>
                  <p
                    className={`text-[9px] font-bold ${results.roi.truePassive < 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
              ...
              <div
                className={`border-4 border-black p-6 ${results.benchmark.isBeatingMarket ? 'bg-green-100' : 'bg-orange-100'}`}
              >
                ...
                <div className="space-y-3 bg-white border-2 border-black p-4 rounded">
                  <div className="flex justify-between items-center pb-2 border-b border-dashed border-gray-300">
                    ...
                    <div className="text-right">
                      ...
                      <p
                        className={`text-sm font-black ${results.roi.truePassive < 0 ? 'text-red-600' : 'text-green-600'}`}
                      >
