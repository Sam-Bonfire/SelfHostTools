...
import { calculateAdminTime as getAdminPercent, calculateFreelanceIncome } from '../lib/freelanceLogic';
import SEO from './SEO';
export default function FreelanceIncomeCalculator() {
  const structuredData = {
    ...
    description:
      'Calculate your true freelance take-home pay after accounting for taxes, overheads, and unpaid admin hours.',
  ...
  const [showAdminEstimator, setShowAdminEstimator] = usePersistedState(
  ...
  const [isPresumptiveTax, setIsPresumptiveTax] = usePersistedState(
    'FreelanceIncomeCalculator',
    'isPresumptiveTax',
    true
  ); // 44ADA
  ...
  const [hardwareSinkingFund, setHardware] = usePersistedState(
    'FreelanceIncomeCalculator',
    'hardwareSinkingFund',
    3000
  );
  ...
  // --- GOALS ---
  const [targetMonthlyIncome, setTargetIncome] = usePersistedState(
    'FreelanceIncomeCalculator',
    'targetMonthlyIncome',
    150000
  );
  ...
  const [projectDirectCosts, setProjectDirectCosts] = usePersistedState(
    'FreelanceIncomeCalculator',
    'projectDirectCosts',
    0
  ); // outsourcing, assets, etc
  ...
  const calculate = useCallback(() => {
    const monthlyExpenses =
      (parseFloat(softwareSaaS) || 0) +
      (parseFloat(hardwareSinkingFund) || 0) +
      (parseFloat(healthInsurance) || 0) +
      (parseFloat(pensionNPS) || 0) +
      (parseFloat(officeRent) || 0) +
      (parseFloat(professionalFees) || 0) +
      (parseFloat(marketingCosts) || 0) +
  ...
  const actions = generateActions(
    'FreelanceIncomeCalculator',
    { isPresumptiveTax, adminTimePercent, hourlyRate, targetMonthlyIncome },
    results
  );
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="FreelanceIncomeCalculator"
            icon={Briefcase}
            title="Freelance Reality Hub"

            onReset={() => {
              resetPersistedState('FreelanceIncomeCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Input
                    id="billable-hours"
                    label="Billable Hrs/Mo"
                    icon={Clock}
                    type="number"
                    value={billableHours}
                    onChange={(e) => setBillableHours(e.target.value)}
                    onBlur={() => !billableHours && setBillableHours(0)}
                    className="font-black w-full"
                  />
                </div>
              </div>
              ...
              <div className="mt-4">
                <Tooltip content="Vacations, Sick days, Public Holidays where you earn ₹0" className="w-full">
                  <Input
                    id="vacation-weeks"
                    label="Unpaid Time Off (Weeks/Yr)"
                    icon={Palmtree}
                    type="number"
                    value={vacationWeeks}
                    onChange={(e) => setVacationWeeks(e.target.value)}
                    onBlur={() => !vacationWeeks && setVacationWeeks(0)}
                    className="font-black text-orange-600 border-orange-200 w-full"
                  />
                </Tooltip>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="mb-1">
                    <Tooltip
                      content="Work you do that you can't bill for: Sales, Invoicing, Emails, Learning skills."
                      className="z-20 inline-block"
                    >
                      <span className="text-[10px] font-black uppercase text-red-600 block border-b border-dashed border-red-300 cursor-help">
                        Unbillable Time (%)
                      </span>
                    </Tooltip>
                  </div>
                  <Input
                    id="admin-time-percent"
                    icon={Clock}
                    type="number"
                    value={adminTimePercent}
                    onChange={(e) => setAdminTime(e.target.value)}
                    onBlur={() => !adminTimePercent && setAdminTime(0)}
                    className="border-red-600 font-black text-red-600 w-full mb-1"
                  />
                  <p className="text-[9px] text-red-400 font-bold leading-tight opacity-70">Standard: 20-30%</p>
                  <button
                ...
                <div>
                  <Input
                    ...
                    onBlur={() => !taxRate && setTaxRate(0)}
                    className="font-black w-full mb-2"
                  />
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                        <label htmlFor={`admin-hours-${item.id}`} className="col-span-4 text-[9px] font-bold uppercase">
                          {item.label}
                        </label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    ...
                    onBlur={() => !softwareSaaS && setSoftware(0)}
                    className="font-black w-full"
                  />
                </div>
                <div>
                  <Input
                    id="expense-hardware"
                    label="Laptop/Gear Fund"
                    icon={IndianRupee}
                    type="number"
                    value={hardwareSinkingFund}
                    onChange={(e) => setHardware(e.target.value)}
                    onBlur={() => !hardwareSinkingFund && setHardware(0)}
                    className="font-black w-full"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    ...
                    onBlur={() => !healthInsurance && setHealth(0)}
                    className="font-black w-full"
                  />
                </div>
                <div>
                  <Input
                    ...
                    onBlur={() => !pensionNPS && setPension(0)}
                    className="font-black w-full"
                  />
                </div>
              </div>
          ...
          <Card title="Tax Strategy" icon={Landmark} headerColor="bg-green-50" className="p-0 border-4 border-black">
            <div className="p-4">
              <div
                className={`p-3 border-2 rounded transition-colors ${isPresumptiveTax ? 'bg-green-100 border-green-600' : 'bg-gray-50 border-gray-200'}`}
              >
                <div className="flex flex-col gap-1">
                  ...
                  <div className="ml-7">
                    <p className="text-xs text-gray-600 leading-relaxed font-medium">
                      Designed for freelancers: You only declare{' '}
                      <strong>50% of your gross receipt as taxable income</strong>. The other 50% is treated as
                      automatic expenses (no proofs needed).
                    </p>
                    {isPresumptiveTax && (
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-6">
          <ResultsAnalysis
            ...
            headerElements={
              <span className="text-xs font-black px-2 py-1 border-2 border-black bg-black text-white">
                MONTHLY VIEW
              </span>
            }
          >
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <MetricDisplay
                    ...
                    value={formatCurrency(results.realHourlyRate)}
                  />
                </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mb-2">
                    To take home <strong>{formatCurrency(targetMonthlyIncome)}</strong>/mo:
                  </p>
                  <div className="flex items-baseline gap-2">
                    <span
                      className={`text-2xl font-black ${results.requiredRate > hourlyRate ? 'text-red-600' : 'text-green-600'}`}
                    >
                      {formatCurrency(results.requiredRate)}
                    </span>
                ...
                <div className="text-right">
                  <Input
                    ...
                    className="w-32 h-8 text-xs font-black border-2"
                  />
                </div>
              </div>
            </div>
            ...
            <Card
              ...
              className="border-4 border-black p-0"
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  ...
                  <div>
                    <Input
                      ...
                      type="number"
                      value={projectBuffer}
                      onChange={(e) => setProjectBuffer(e.target.value)}
                      className="font-black w-full"
                    />
                  </div>
                  <div>
                    <Input
                      id="project-direct-costs"
                      label="Direct Costs"
                      icon={IndianRupee}
                      type="number"
                      value={projectDirectCosts}
                      onChange={(e) => setProjectDirectCosts(e.target.value)}
                      className="font-black w-full"
                    />
                  </div>
                </div>
