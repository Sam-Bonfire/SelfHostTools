...
import { BASKET_ITEMS, calculateInflationDestroyer, HISTORICAL_REGIMES } from '../lib/inflationLogic';
import SEO from './SEO';
...
export default function InflationDestroyer() {
  const structuredData = {
    ...
    description:
      'Calculate how inflation destroys purchasing power. Translate cash erosion into real-world goods equivalents and compare against investing.',
  ...
  const [customInflationRate, setCustomInflationRate] = usePersistedState(
    'InflationDestroyer',
    'customInflationRate',
    6
  );
  ...
  const [selectedBasketId, setSelectedBasketId] = usePersistedState(
    'InflationDestroyer',
    'selectedBasketId',
    'groceries'
  );
  ...
  const erosionBarWidth = results ? Math.min(100, results.results.erosionPercent) : 0;
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <CalculatorHeader
          namespace="InflationDestroyer"
          icon={Flame}
          title="Inflation Destroyer"
          onReset={() => {
            resetPersistedState('InflationDestroyer');
          }}
        />
            ...
            <div className="space-y-5">
              <div>
                <Input
                  ...
                  onBlur={() => !principal && setPrincipal(0)}
                  className="font-black"
                />
                <p className="text-[10px] text-gray-500 mt-2 font-bold uppercase leading-none">
                  Savings sitting in a fixed deposit, savings account, or under the mattress
                </p>
              </div>
          ...
          <Card title="Inflation Regime" icon={Flame} headerColor="bg-red-100">
            <div className="space-y-3">
              {HISTORICAL_REGIMES.map((regime) => (
                ...
                >
                  <div
                    className={`w-4 h-4 mt-0.5 border-2 border-black flex-shrink-0 ${regimeId === regime.id ? 'bg-black' : 'bg-white'}`}
                  />
                  <div>
                  ...
                  >
                    <div className="pt-2">
                      <Input
                        ...
                        onBlur={() => !customInflationRate && setCustomInflationRate(6)}
                        className="font-black"
                      />
                    </div>
                  </motion.div>
          ...
          <Card title="Investment Comparison" icon={TrendingUp} headerColor="bg-green-100">
            <div className="space-y-4">
              <p className="text-[10px] text-gray-500 font-bold uppercase leading-tight">
                Compare your cash erosion against investing the same amount in a market index fund
              </p>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Input
                    ...
                    onBlur={() => !investmentTaxRate && setInvestmentTaxRate(0)}
                    className="font-black"
                  />
                </div>
              </div>
              ...
              >
                {BASKET_ITEMS.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.label} (₹{item.unitCost.toLocaleString('en-IN')})
                  </option>
                ))}
              </Select>
            </div>
            ...
            <ResultsAnalysis>
              ...
              <div className="border-4 border-black p-5 bg-gray-50">
                <h3 className="text-xs font-black uppercase mb-3 flex items-center gap-2">
                  <Flame className="w-4 h-4 text-red-600 animate-pulse" /> Purchasing Power Erosion —{' '}
                  {results.results.erosionPercent}% Gone
                </h3>
                <div className="w-full h-8 bg-gray-200 border-2 border-black overflow-hidden shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  <div className="flex h-full">
                    ...
                    >
                      <span className="text-[10px] font-black uppercase whitespace-nowrap overflow-hidden">
                        Remaining Power
                      </span>
                    </motion.div>
                    <motion.div
                    ...
                    >
                      <span className="text-[10px] font-black uppercase text-white whitespace-nowrap overflow-hidden">
                        Destroyed
                      </span>
                    </motion.div>
                  </div>
                ...
                <div className="flex justify-between mt-1">
                  <span className="text-[10px] font-bold text-green-700">
                    {formatCurrency(results.results.finalPurchasingPower)} real value
                  </span>
                  <span className="text-[10px] font-bold text-red-600">
                    -{formatCurrency(results.results.erosionAmount)} lost
                  </span>
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="bg-white border-2 border-black p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <MetricDisplay
                      ...
                      value={results.basket.unitsToday}
                      subtitle={results.basket.unit}
                    />
                  </div>
                  <div className="bg-red-100 border-2 border-black p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <MetricDisplay
                      title="Lost to Inflation"
                      value={`-${results.basket.unitsLost}`}
                      subtitle={results.basket.unit}
                    />
                  </div>
                  <div className="bg-white border-2 border-black p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <MetricDisplay
                      title={`Buying Power in Yr ${years}`}
                      value={results.basket.unitsFuture}
                      subtitle={results.basket.unit}
                    />
                  </div>
                </div>
              ...
              <div className="border-4 border-black p-5 bg-gray-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="p-4 bg-red-50 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                    <MetricDisplay
                      title="Cash (No Investment)"
                      value={formatCurrency(results.results.finalPurchasingPower)}
                      subtitle="Real buying power after inflation"
                    />
                  </div>
                  <div
                    className={`p-4 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] ${results.results.isBeatingInflation ? 'bg-green-50' : 'bg-orange-50'}`}
                  >
                    <MetricDisplay
                      title="Invested (Real After-Tax Value)"
                      value={formatCurrency(results.results.realInvestmentValue)}
                      subtitle="Inflation-adjusted after tax on gains"
                    />
                  </div>
                </div>
                ...
                {/* Verdict */}
                <div
                  className={`p-3 border-2 border-black flex items-center gap-3 ${results.results.isBeatingInflation ? 'bg-green-100' : 'bg-red-100'}`}
                >
              ...
              <div>
                <Tooltip content="View annual decay schedule" className="w-full">
                  ...
                  >
                    ...
                    {showSchedule ? 'Hide Year-by-Year Decay' : 'View Year-by-Year Decay Table'}
                  </Button>
                </Tooltip>
                              ...
                              {results.schedule.map((row) => (
                                <tr key={row.label} className="hover:bg-yellow-50 transition-colors">
                                  ...
                                  <td className="px-4 py-3 text-red-600 font-mono">
                                    {formatCurrency(row.cashPurchasingPower)}
                                  </td>
                                  <td className="px-4 py-3 text-green-600 font-mono">
                                    {formatCurrency(row.netInvestmentValue)}
                                  </td>
                                  <td className="px-4 py-3 text-right font-mono font-bold">
                                    {formatCurrency(row.realInvestmentValue)}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          <strong>Disclaimer:</strong> Inflation projections are estimates based on selected regimes. Real CPI varies by
          household spending pattern. Investment returns are historical averages and not guaranteed.
        </p>
      </Footer>
    </div>
  );
