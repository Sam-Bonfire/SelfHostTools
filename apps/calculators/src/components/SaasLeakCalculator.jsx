...
export default function SaasLeakCalculator() {
  const structuredData = {
    ...
    description:
      'Calculate the true opportunity cost of your monthly subscriptions. See how compounding returns and active work hours drain your long-term wealth.',
    brand: {
      '@type': 'Brand',
  ...
  const [newSubBillingPeriod, setNewSubBillingPeriod] = usePersistedState(
    'SaasLeakCalculator',
    'newSubBillingPeriod',
    'monthly'
  );
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="SaasLeakCalculator"
            icon={Flame}
            title="SaaS Subscription Leak Realist"

            onReset={() => {
              resetPersistedState('SaasLeakCalculator');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
          ...
          {calcMode === 'individual' && (
            <Card
              title="Monthly Subscriptions Spend"
              icon={IndianRupee}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                  {subscriptions.map((sub) => {
                    ...
                    return (
                      <div
                        key={sub.id}
                        className="flex items-center justify-between p-3 bg-gray-50 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                      >
                        <div className="flex items-center gap-3">
                          <Checkbox
                            checked={sub.active}
                            onChange={() => handleToggleSubActive(sub.id)}
                            aria-label={`Toggle ${sub.name}`}
                          />
                          <div>
                            <span
                              className={`text-xs font-black uppercase ${sub.active ? 'text-black' : 'text-gray-400 line-through'}`}
                            >
                              {sub.name}
                            </span>
                  ...
                  {subscriptions.length === 0 && (
                    <div className="text-center py-6 text-gray-500 text-xs font-bold uppercase">
                      No subscriptions added. Add one below!
                    </div>
                  )}
                </div>
                ...
                <div className="border-t-2 border-black/10 pt-4 space-y-3">
                      ...
                      >
                        {USAGE_FREQUENCIES.map((freq) => (
                          <option key={freq.id} value={freq.id}>
                            {freq.label} (approx. {freq.usesPerMonth} uses/mo)
                          </option>
                        ))}
                      </Select>
                    </div>
                  ...
                  <Button
                    ...
                    variant="primary"
                    className="w-full text-xs font-black uppercase border-2 flex items-center justify-center gap-2"
                  >
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-6">
          <ResultsAnalysis>
            ...
            <div className="bg-red-600 border-4 border-black text-white p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4 w-full">
                ...
                <div className="flex-1">
                  <p className="text-red-100 text-xs font-black uppercase tracking-widest">
                    30-Year Compounded Lost Wealth
                  </p>
                  <motion.div
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 border-t border-white/20 pt-2 text-[10px] font-bold uppercase tracking-tight text-red-100">
                    <span>
                      Monthly Spend:{' '}
                      <strong className="text-white">{formatCurrency(results.results.totalMonthlySpend)}/mo</strong>
                    </span>
                    <span className="hidden md:inline">•</span>
                    <span>
                      Annual Spend:{' '}
                      <strong className="text-white">{formatCurrency(results.results.annualSpend)}/yr</strong>
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card
                title="Annual Active Labor Hours"
                icon={Clock}
                headerColor="bg-white"
                className="flex flex-col justify-between h-full"
              >
                <p className="text-[10px] text-gray-500 mb-4 uppercase font-bold tracking-tight leading-none">
                  Working hours required purely to fund these subscriptions
                </p>
                <MetricDisplay
              ...
              {/* Work Days Needed */}
              <Card
                title="Equivalent Work Days"
                icon={Calendar}
                headerColor="bg-white"
                className="flex flex-col justify-between h-full"
              >
                <p className="text-[10px] text-gray-500 mb-4 uppercase font-bold tracking-tight leading-none">
                  Days of labor dedicated entirely to subscription companies
                </p>
                <MetricDisplay
            ...
            {hourlyWage <= 0 && (
              <div className="bg-[#FF6B6B]/10 border-4 border-black p-4 flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-sm font-black uppercase text-red-950 leading-tight">
            ...
            {results.deadWeightItems && results.deadWeightItems.length > 0 && (
              <Card
                title="Dead Weight Subscriptions Detected!"
                icon={<ShieldAlert className="w-5 h-5 md:w-6 md:h-6 text-red-600 animate-pulse" />}
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {results.deadWeightItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex justify-between items-center bg-white p-2 border-2 border-black text-xs font-bold uppercase"
                      >
                        <span className="text-black">{item.name}</span>
                        <span className="text-red-600 font-mono">
                          {formatCurrency(item.cost)}/{item.billingPeriod === 'yearly' ? 'yr' : 'mo'}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {results.categoryBreakdown.map((cat) => (
                      <div
                        key={cat.id}
                        className="border-2 border-black p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] bg-gray-50 flex flex-col justify-between"
                      >
                        <div className="flex justify-between items-start">
                          <span className="text-xs font-black uppercase" style={{ color: cat.color }}>
                            {cat.label}
                          </span>
            ...
            {calcMode === 'aggregate' && subscriptions.filter((s) => s.active).length > 0 && (
              <Card
                title="SaaS Efficiency & Cost-Per-Use Audit"
                icon={Award}
                      ...
                      <tbody className="divide-y border-black/10">
                        {subscriptions
                          ...
                          .map((sub) => {
                            ...
                            const freq = USAGE_FREQUENCIES.find((f) => f.id === sub.usageFrequency) || {
                              label: 'Monthly'
                            };
                            return (
                              <tr
                                key={sub.id}
                                className="hover:bg-emerald-50/50 transition-colors uppercase font-bold text-[10px]"
                              >
                                <td className="px-3 py-2 text-black">{sub.name}</td>
                                <td className="px-3 py-2 text-gray-500">{freq.label}</td>
                                <td className="px-3 py-2 text-center">
                                  <span
                                    className={`px-1 text-[9px] text-white ${metrics.valueScore >= 7 ? 'bg-green-600' : metrics.valueScore >= 4 ? 'bg-yellow-500' : 'bg-red-500'}`}
                                  >
                                    {metrics.valueScore}/10
                                  </span>
                                </td>
                                <td className="px-3 py-2 text-right font-mono text-black">
                                  {formatCurrency(metrics.costPerUse)}
                                </td>
                              </tr>
                            );
                          })}
            ...
            <div className="border-4 border-black p-6 bg-yellow-50">
              ...
              <p className="text-xs text-gray-600 font-bold mb-6 uppercase leading-tight">
                If you diverted ₹{results.results.totalMonthlySpend}/mo to a NIFTY 50 index fund compounding at{' '}
                {expectedReturn}% per annum, here is the net wealth accumulated:
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-white border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-36">
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-500 uppercase mt-2">
                      Principal: {formatCurrency(results.projections[10].principal)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xl font-black text-black">
                      {formatCurrency(results.projections[10].futureValue)}
                    </p>
                    <p className="text-[9px] text-green-600 font-bold uppercase">
                      Compound: +{formatCurrency(results.projections[10].compoundReturns)}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-4 bg-white border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-36">
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-500 uppercase mt-2">
                      Principal: {formatCurrency(results.projections[20].principal)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xl font-black text-orange-600">
                      {formatCurrency(results.projections[20].futureValue)}
                    </p>
                    <p className="text-[9px] text-green-600 font-bold uppercase">
                      Compound: +{formatCurrency(results.projections[20].compoundReturns)}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-4 bg-white border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-36">
                  <div>
                    ...
                    <p className="text-xs font-bold text-gray-500 uppercase mt-2">
                      Principal: {formatCurrency(results.projections[30].principal)}
                    </p>
                  </div>
                  <div>
                    <p className="text-2xl font-black text-red-600">
                      {formatCurrency(results.projections[30].futureValue)}
                    </p>
                    <p className="text-[9px] text-green-600 font-bold uppercase">
                      Compound: +{formatCurrency(results.projections[30].compoundReturns)}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-white border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold uppercase text-green-600">
                        Lost Interest (Wealth Compound)
                      </span>
                    </div>
                    <span className="font-mono font-black text-green-600">
                      +{formatCurrency(results.projections[30].compoundReturns)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div>
              <Tooltip content="View annual compound trajectory" className="w-full">
                ...
                >
                  ...
                  {showSchedule ? 'Hide Yearly Projections' : 'View Year-by-Year Compound Projections'}
                </Button>
              </Tooltip>
            ...
            <div className="bg-gray-50 border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h4 className="text-xs font-black uppercase text-black leading-none">
                  Export Subscription Leak Analysis
                </h4>
                <p className="text-[10px] text-gray-500 font-bold uppercase mt-1 tracking-tight leading-none">
                  Take your offline financial audit reports home
                </p>
              </div>
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          <strong>Disclaimer:</strong> Real returns are calculated based on historical index averages and are subject to
          market volatility. The work hour equivalent assumes standard payroll taxes are already deducted from the net
          hourly wage.
        </p>
      </Footer>
    </div>
