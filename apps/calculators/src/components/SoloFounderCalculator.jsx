...
export default function SoloFounderCalculator() {
  ...
  const [averageRevenuePerUser, setAverageRevenuePerUser] = usePersistedState(
    'SoloFounderCalculator',
  ...
  const [weeklyHoursDedicated, setWeeklyHoursDedicated] = usePersistedState(
    'SoloFounderCalculator',
    'weeklyHoursDedicated',
    20
  );
  ...
  const freedomProgress =
    milestones.freedomMRR > 0 && milestones.freedomMRR !== Infinity
      ? Math.min(100, (mrr / milestones.freedomMRR) * 100)
      : 0;
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 space-y-6">
              <Card
                ...
                className="shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-white"
              >
            ...
            <div className="lg:col-span-7 space-y-6">
              <ResultsAnalysis
                ...
                verdict={
                  milestones.hasReachedFreedom
                    ? 'You have reached Freedom MRR! Time to quit.'
                    : milestones.isProfitable
                      ? 'Profitable, but keep grinding towards Freedom.'
                      : 'Currently burning cash. Focus on reaching Break-Even.'
                }
                verdictColor={
                  milestones.hasReachedFreedom
                    ? 'text-green-600'
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  <div
                    className={`p-4 border-4 border-black ${milestones.isProfitable ? 'bg-emerald-50' : 'bg-rose-50'}`}
                  >
                    <MetricDisplay
                      title={
                ...
                {metrics.marginPerUserPercent <= 0 && (
                  <div className="mt-6 p-4 bg-red-100 border-4 border-red-500 text-red-800 font-bold flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        Your unit economics are fundamentally broken. Your churn and transaction fees consume 100%+ of
                        your revenue. You can never reach Break-Even no matter how many users you acquire. Raise prices
                        or lower variable costs immediately.
                      </p>
                    </div>
                  </div>
                )}
                {metrics.marginPerUserPercent > 0 && (
                  <div className="mt-6 p-4 bg-purple-50 border-2 border-purple-200 text-purple-800 font-medium text-sm">
                    <p>
                      <strong>Did you know?</strong> Your actual profit margin per active user is exactly{' '}
                      <strong>{metrics.marginPerUserPercent.toFixed(1)}%</strong>. Every new{' '}
                      {formatCurrency(averageRevenuePerUser)} user only adds{' '}
                      {formatCurrency(averageRevenuePerUser * (metrics.marginPerUserPercent / 100))} to your bottom line
                      before taxes.
                    </p>
                  </div>
                )}
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          ...
          Never mistake MRR for personal income. After churn, gateway fees, servers, tools, and taxes, the real money
          you take home is often less than half of your topline.
        </p>
      </Footer>
    </div>
  );
