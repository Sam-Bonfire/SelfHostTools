export function calculateSoloFounderRunway({
  ...
  const marginPerUserPercent =
    1 - churnRatePercent / 100 - stripeFeePercent / 100 - stripeFixedFee / averageRevenuePerUser;
  ...
  return {
    ...
    metrics: {
      trueHourlyRate,
      customersNeededForFreedom:
        freedomMRR > 0 && freedomMRR !== Infinity ? Math.ceil(freedomMRR / averageRevenuePerUser) : 0,
