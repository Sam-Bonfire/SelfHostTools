export const calculateLifeInsurance = ({
    ...
    if (Array.isArray(futureGoals)) {
        futureGoals.forEach(g => {
            const futureCost = (parseFloat(g.amount) || 0) * Math.pow(1 + ((parseFloat(inflationRate) || 0) / 100), (parseFloat(g.yearsAway) || 0));
            const pvRequired = futureCost / Math.pow(1 + ((parseFloat(investmentReturn) || 0) / 100), (parseFloat(g.yearsAway) || 0));
            goalCover += pvRequired;
        });
    }

    const totalRequired = Math.round(expenseCover + goalCover + (parseFloat(liabilities) || 0) - (parseFloat(existingAssets) || 0));
