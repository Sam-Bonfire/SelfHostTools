...
export const calculateSinkingFund = (items, inflationRate = 5) => {
  ...
  return {
    ...
    itemDetails,
    immediateLiability: items
      .filter((i) => i.currentAgeYears >= i.lifespanYears)
      .reduce((acc, i) => acc + i.replacementCost, 0)
  };
};
...
export const calculateHomeOwnerRealism = ({
  ...
  const { totalMonthlySinkingFund, itemDetails, immediateLiability } = calculateSinkingFund(
    auditItems,
