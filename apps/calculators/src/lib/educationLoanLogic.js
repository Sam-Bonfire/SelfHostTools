export const calculateEducationLoan = ({
  ...
  if (effectivePrincipal > 0 && originalTenureMonths > 0) {
    emi =
      (effectivePrincipal * ratePerMonth * Math.pow(1 + ratePerMonth, originalTenureMonths)) /
  ...
  if (capitalizeInterest) {
    ...
    const e =
      (pWithInt * ratePerMonth * Math.pow(1 + ratePerMonth, originalTenureMonths)) /
      (Math.pow(1 + ratePerMonth, originalTenureMonths) - 1);
    baselineTotal = e * originalTenureMonths;
  } else {
    const e =
      (originalDisbursed * ratePerMonth * Math.pow(1 + ratePerMonth, originalTenureMonths)) /
      (Math.pow(1 + ratePerMonth, originalTenureMonths) - 1);
