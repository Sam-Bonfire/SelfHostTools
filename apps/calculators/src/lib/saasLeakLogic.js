...
export const calculateSaaSLeak = ({
  ...
  const buildProjection = (years) => {
    ...
    const futureValue = r > 0 ? totalMonthlySpend * ((Math.pow(1 + r / 12, years * 12) - 1) / (r / 12)) : principal;
    return {
      years,
  ...
  const schedule = Array.from({ length: 30 }, (_, index) => {
    ...
    const futureValue = r > 0 ? totalMonthlySpend * ((Math.pow(1 + r / 12, y * 12) - 1) / (r / 12)) : principal;
  ...
  subscriptions
    ...
    .forEach((sub) => {
      ...
      const costPerUse = freq.usesPerMonth > 0 ? Math.round((subMonthly / freq.usesPerMonth) * 10) / 10 : subMonthly;
  ...
  const deadWeightItems = subscriptions.filter((s) => s.active && perSubMetrics[s.id]?.isDeadWeight);
