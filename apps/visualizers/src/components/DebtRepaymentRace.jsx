...
export default function DebtRepaymentRace() {
    ...
    const debtColors = useMemo(() => {
        const colors = ['bg-red-400', 'bg-blue-400', 'bg-yellow-400', 'bg-purple-400', 'bg-pink-400', 'bg-orange-400', 'bg-teal-400'];
        const colorMap = {};
        debts.forEach((d, i) => { colorMap[d.id] = colors[i % colors.length]; });
        return colorMap;
    }, [debts]);
            ...
            <CalculatorLayout>
                ...
                <div className="lg:col-span-12 xl:col-span-4 space-y-6">
                    ...
                    <Card className="p-5 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-yellow-50">
                        <span className="block text-[10px] font-black uppercase text-gray-500 mb-2">
                            Load Quick Presets
                        </span>
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-600 font-bold uppercase">
                                            ₹{d.balance.toLocaleString('en-IN')} @ {d.interestRate}% • min ₹
                                            {d.minPayment}
                                        </p>
                                    </div>
                                    <button
                            ...
                            <Card className="p-6 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] space-y-6">
                                ...
                                <div className="p-4 bg-gray-50 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-xs font-black uppercase">Month {timelineMonth} of {maxMonths}</span>
                                        ...
                                        <div className="h-full flex-1 flex transition-all duration-200 shrink-0">
                                            {racePositions.avBalances.map((db) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        {relativeWidth > 15 && <span className="text-[10px] font-black uppercase truncate px-1">{db.name}</span>}
                                                    </div>
                                                );
                                            })}
                                        </div>
