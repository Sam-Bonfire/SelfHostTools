...
const SCENARIOS = {
  depression: {
    ...
    startYear: 1929,
    returns: [-8.4, -24.9, -43.3, -8.1, 53.9, -1.4, 47.6, 33.9, -35.0, 31.1, -0.4, -9.7, -11.5, 20.3, 25.9, 19.7, 36.4, -8.0, 5.7, 5.5, 18.7, 31.7, 24.0, 18.3, -1.0, 52.6, 31.5, 6.5, -10.7, 43.3],
    inflation: [0, -2.3, -9.0, -9.9, -5.1, 3.1, 2.2, 1.5, 3.6, -2.1, -1.4, 0.7, 5.0, 10.9, 6.1, 1.7, 2.3, 8.3, 14.4, 8.1, -1.2, 1.3, 7.9, 1.9, 0.8, 0.7, -0.4, 1.5, 3.3, 2.8]
  },
  stagflation: {
    name: '1968 Stagflation',
    description: 'High inflation eroding real value despite nominal market gains.',
    startYear: 1968,
    returns: [11.0, -8.5, 4.0, 14.3, 19.0, -14.6, -26.4, 37.2, 23.6, -7.1, 6.5, 18.6, 32.5, -4.9, 21.5, 22.5, 6.2, 31.7, 18.6, 5.2, 16.6, 31.6, -3.1, 30.4, 7.6, 10.0, 1.3, 37.5, 22.9, 33.3],
    inflation: [4.2, 5.4, 5.9, 4.3, 3.2, 6.2, 11.0, 9.1, 5.8, 6.5, 7.6, 11.3, 13.5, 10.3, 6.1, 3.2, 4.3, 3.5, 1.9, 3.6, 4.1, 4.8, 5.4, 4.2, 3.0, 3.0, 2.6, 2.8, 2.9, 2.3]
  },
  dotcom: {
    name: '2000 Dot-com & GFC',
    description: 'Two massive crashes within a decade testing early sequence of returns.',
    startYear: 2000,
    returns: [-9.1, -11.8, -22.1, 28.6, 10.8, 4.9, 15.7, 5.4, -37.0, 26.4, 15.0, 2.1, 16.0, 32.3, 13.6, 1.3, 11.9, 21.8, -4.3, 31.4, 18.4, 28.7, -18.1, 24.2, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0],
    inflation: [3.3, 2.8, 1.5, 2.2, 2.6, 3.3, 3.2, 2.8, 3.8, -0.3, 1.6, 3.1, 2.0, 1.4, 1.6, 0.1, 1.2, 2.1, 2.4, 1.8, 1.2, 4.7, 8.0, 4.1, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0]
  }
};
              ...
              {results.depletedYear ? (
                <div className="bg-red-200 border-4 border-black p-4 flex items-start gap-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <div>
                    ...
                    <p className="text-red-800 font-medium mt-1">Ran out of money in <strong>{results.depletedYear}</strong>.</p>
                  </div>
                </div>
              ) : (
                <div className="bg-green-200 border-4 border-black p-4 flex items-start gap-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <div>
                    ...
                    <p className="text-green-800 font-medium mt-1">Ending Balance: <strong>{formatCurrency(results.finalValue)}</strong></p>
                  </div>
                </div>
              )}
        ...
        <div className="col-span-1 lg:col-span-8 flex flex-col gap-6">
              ...
              <div className="flex-grow w-full h-full relative" style={{ minHeight: '300px' }}>
                <svg className="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 300 100">
                  ...
                  <polygon
                    ...
                    points={`0,100 ${results.path.map((d, i) => {
                      const x = (i / 29) * 300;
                      const y = 100 - ((d.portfolio / maxPortfolio) * 100);
                      return `${x},${y}`;
                    }).join(' ')} 300,100`}
                  ...
                  {results.path.map((d, i) => {
                    ...
                    const y = 100 - ((d.portfolio / maxPortfolio) * 100);
                    return (
                      <circle key={i} cx={x} cy={y} r="1.5" fill={d.portfolio === 0 ? "#ef4444" : "#000000"} />
                    );
                  })}
                </svg>
          ...
          <Card title="Why does this happen?" className="bg-blue-100">
            <p className="font-medium text-gray-800 leading-relaxed">
              The sequence of returns risk is the danger that the timing of withdrawals from a retirement account will have a negative impact on the overall rate of return. 
              Even if average returns are positive, withdrawing during a prolonged down market (like 1929) or during high inflation (like the 1970s) forces you to sell more shares to meet your living expenses, permanently impairing the portfolio's ability to recover.
            </p>
          </Card>
        </div>
