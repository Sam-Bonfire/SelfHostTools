...
import { Button, CalculatorHeader, CalculatorLayout, Card, Footer, ResultsAnalysis } from '@packages/styling';
import { clsx } from 'clsx';
import { Box, FastForward, MousePointer2, Pause, Play, RotateCcw } from 'lucide-react';
...
const PRESETS = [
  ...
  {
    id: 'replicator',
    name: 'Replicator (Fractal)',
    rule: 'B1357/S1357',
    desc: 'Replaces itself, creating Sierpinski triangle patterns.'
  },
    ...
    <div className="min-h-screen bg-white text-black p-4 md:p-8 font-sans">
      ...
      <CalculatorLayout>
        <div className="lg:col-span-12">
          <CalculatorHeader
            namespace="CellularAutomataVisualizer"
            icon={Box}
            title="Cellular Automata"

            onReset={() => {
              resetPersistedState('CellularAutomataVisualizer');
            }}
          />
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-4 space-y-6">
          <Card title="Controls" icon={Box} headerColor="bg-yellow-300">
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {PRESETS.map((p) => (
                    ...
                    >
                      ...
                      {preset.id === p.id && (
                        <p className="text-[10px] mt-1 opacity-80 normal-case font-medium">{p.desc}</p>
                      )}
                    </button>
                  ))}
                </div>
              ...
              {/* Speed Slider */}
              <div>
                <label className="block text-xs font-black uppercase mb-2">Simulation Speed: {speed} FPS</label>
                <input
                  type="range"
                  min="1"
                  max="60"
      ...
      <Footer>
        <p className="text-gray-600 font-medium">
          <strong>Disclaimer:</strong> Cellular automata demonstrate how staggering complexity emerges from profoundly
          simple rules.
